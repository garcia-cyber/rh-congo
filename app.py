...
def home():
    if 'rh' in session:
        ...
        with sqlite3.connect("rh.db") as con :
            #compte le nombre de user
            cur = con.cursor()
            cur.execute("select * from employes")
            aff = len(cur.fetchall())

            #nombre de filles
            fille = con.cursor()
            fille.execute("select * from employes where sexeEmpl = 'F'")
            aff_fille = len(fille.fetchall())

            #nombre des hommes
            homme = con.cursor()
            homme.execute("select * from employes where sexeEmpl = 'M'")
            aff_homme = len(homme.fetchall())


        return render_template('index.html', aff = aff , aff_fille = aff_fille , aff_homme = aff_homme )
    else:
        return redirect('/')
...
##
#
# Modification du personnel par le responsable des ressource humaine
@app.route('/modifiePersonnel/<string:idEmpl>', methods =['POST', 'GET'])
def modifiePersonnel(idEmpl):
    if 'rh' in session:
        if request.method =='POST':
            ...
            with sqlite3.connect("rh.db") as con :
                if True:
                    cur = con.cursor()
                    cur.execute("""
                                    update employes set nomsEmpl = ?,matriculeEmpl = ?,civiliteEmpl = ?,sexeEmpl = ?
                                    ,phoneEmpl = ?,communeEmpl = ?,adresseEmpl = ?,lieuNai = ?,dateNai = ?,province = ?,
                                posteEmpl = ?,register = ? where idEmpl = ?

""",[noms,matricule,etat,sexe,telephone,commune,avenue,lieu,naissance,province,poste,session['id'], idEmpl])
                    con.commit()
                    cur.close()
        ...
        with sqlite3.connect('rh.db') as con :
            all = con.cursor()
            all.execute("select * from employes where idEmpl = ?" , [idEmpl])
            data_all = all.fetchone()

            #table poste
            pst = con.cursor()
            pst.execute("select * from postes")
            dataPst = pst.fetchall()

        return render_template('modifierPersonnel.html', dt = dataPst ,data = data_all)
    else:
        return redirect('/login')
###
#
...
def absence(idEmpl):
    if 'rh' in session:
        if request.method == 'POST':
            ...
            with sqlite3.connect('rh.db') as con :
                vr = con.cursor()
                vr.execute("select * from presences where  emplID = ? and jourP = ?",[idEmpl,jour])
                data = vr.fetchone()

                if data:
                    flash("information existe deja")
                else:
                    cur = con.cursor()
                    cur.execute("insert into presences(jourP,emplID) values(?,?)",[jour,idEmpl])
                    con.commit()
                    cur.close()
